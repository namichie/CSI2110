 ...
 * inner class), It also provide a series of methods for encoding and decoding.
 * It uses a BitFeedOut which allows a stream of bits be sent continuously to be
 * used for encoding. It also uses an Iterator<Byte> which allows a stream of
 * bits to be read continuously to be used when decoding.
 * 
 * @author Lucia Moura
 */
...
public class HuffmanTree {
	...
	public static int EndOfText = ((int) '\uffff') + 1; // special symbol
														// created to indicate
														// end of text

	HuffmanNode root = null; // root of the Huffman tree
	HuffmanNode[] leafWhereLetterIs; // array indexed by characters, storing a
										// reference to
										// the Huffman Node (leaf) in which the
										// character is stored
	...
	private HuffmanNode BuildTree(int[] frequencies, char[] letters) {
		...
		HeapPriorityQueue<HuffmanNode, HuffmanNode> heap = new HeapPriorityQueue<HuffmanNode, HuffmanNode>(
		...
		while (heap.size() > 1) {
			...
			// Subtrees
			HuffmanNode left = e1.getValue();
			HuffmanNode right = e2.getValue();
			
			// Build new tree w/ left and right subtrees
			HuffmanNode parent = new HuffmanNode(0, 0, null, left, right);
			
			// Combine frequencies
			int f1 = e1.getKey().getFrequency();
			int f2 = e2.getKey().getFrequency();
			int newFreq = f1 + f2;
			parent.setFrequency(newFreq);
	...
	private String encodeCharacter(int c) { 
		 ...
		 * Remember that the algorithm must run in O(L) where L is the size of
		 * the codeword generated
		 ****/
		...
		while (current.parent() != null) {
			HuffmanNode leftSibling = current.parent().leftChild();
			HuffmanNode rightSibling = current.parent().rightChild();
			
			if (leftSibling != null && leftSibling == current) {
				// current is left child of parent
				encoding = "0" + encoding;
				
			} else if (rightSibling != null && rightSibling == current) {
	...
	public void encodeCharacter(int c, BitFeedOut bfo) {
		...
		for (int i = 0; i < s.length(); i++)
			bfo.putNext(s.charAt(i));
	}

	
	// decodeCharacter receives Iterator<Byte> bit that iterates through a
	// sequence of bits of the encoded string; this sequence must be
	...
	// decodeCharacter will apply bit.next() 3 times until it decodes
	...
	public int decodeCharacter(Iterator<Byte> bit) {
		...
		/***
		 * 
		 * Decodes sequence of next bits returned by several calls to bit.next()
		 * until it completes the decoding of the next character
		 * 
		 * Note1: the return value of bit.next() is a Byte but should be
		 * interpreted as a bit; it must be either 0 or 1. 
		 * Note2: the return value is an integer (unicode) of the character; for a character char
		 * c this can be obtained by casting the character: (int) c
		 * 
		 * Remember that the algorithm must run in O(L) where L is the size of
		 * the codeword for the character
		 * 
		 * 
		 */
		if (root == null) {
			return Integer.MAX_VALUE;// empty tree is not valid when decoding
		}
	...
	public class HuffmanNode implements Comparable<HuffmanNode> {
		...
		void setLeftChild(HuffmanNode leftVal) {
			left = leftVal;
		}

		void setRightChild(HuffmanNode rightVal) {
			right = rightVal;
		}

		void setParent(HuffmanNode parentVal) {
			parent = parentVal;
		}

		void setLetter(char letterVal) {
			letter = letterVal;
		}

		void setFrequency(int freqVal) {
			frequency = freqVal;
		}
