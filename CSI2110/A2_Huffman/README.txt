...
9. Create new binary tree newT with left subtree e1.tree and right subtree e2.tree // O(d) -> O(d^2)
...
Calculation: 3O(1) + 6O(d) + 3O(d^2) = O(d^2)

The running time is O(d^2).
...
Calculation: 2O(1) + 7O(d) + 3O(d^2) = O(d^2)
...
Calculation: 2O(1) + 5O(d) + 4O(dlog(d)) + O(log(d)) = O(dlog(d))

The running time is O(dlog(d)).